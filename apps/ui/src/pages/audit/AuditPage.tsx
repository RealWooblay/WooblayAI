...
export function AuditPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {chain && (
        <div className={`rounded-xl border p-4 flex items-center gap-4 ${
          ...
          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xl ${
            chain.chainValid ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'
          }`}>
            {chain.chainValid ? '✓' : '✗'}
          </div>
          <div>
            <p className={`text-sm font-medium ${chain.chainValid ? 'text-emerald-400' : 'text-red-400'}`}>
              {chain.chainValid ? 'Chain Verified' : 'Chain Integrity Issues'}
            </p>
            <p className="text-xs text-text-muted">
              {chain.totalReceipts} receipts &middot; {chain.hashesVerified} hashes verified
              {chain.gaps.length > 0 && ` · ${chain.gaps.length} gaps detected`}
            </p>
          </div>
        </div>
      )}
      ...
      {/* Report Table */}
      {isLoading && (
        <div className="p-8 text-center text-text-muted text-sm animate-pulse">Generating report...</div>
      )}
        ...
        <div className="bg-surface-1 border border-border rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between">
            ...
            <span className="text-[10px] text-text-muted">
              {report.period.from.slice(0, 10)} &rarr; {report.period.to.slice(0, 10)}
            </span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-border">
                {report.rows.map((row) => (
                  <tr key={row.receiptId} className="hover:bg-surface-2">
                    ...
                    <td className="px-3 py-2">
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${riskBadge[row.riskTier] ?? ''}`}>
                        {row.riskTier}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-text-primary">{row.policyDecision}</td>
                    <td className="px-3 py-2 text-text-primary">
                      {row.approvalStatus ?? '—'}
                      {row.approver && <span className="text-text-muted ml-1">({row.approver})</span>}
                    </td>
                    <td className="px-3 py-2 font-mono text-[9px] text-text-muted">
                      {row.receiptHash.slice(0, 16)}...
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {report && report.rows.length === 0 && (
        <div className="bg-surface-1 border border-border rounded-xl p-8 text-center text-text-muted text-sm">
