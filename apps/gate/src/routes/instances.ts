...
function writeInstanceCompose(
  ...
  const hasMcpProxy = enabledServers.length > 0;
...
services:
  ...
  if (hasMcpProxy) {
    // Serialize MCP server configs for the proxy sidecar.
    // Only IDs and transport details — no credentials. The Gate resolves
    // creds from vault connections at L3 exec time inside ephemeral containers.
    ...
    // Write proxy env file — includes GATEWAY_TOKEN for defense-in-depth auth
    ...
    // Inject proxy SSE URL into the agent container's env
...
export async function instanceRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/instances', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      writeInstanceCompose(dir, name, port, undefined, gatewayToken);
  ...
  app.post('/api/instances/:id/start', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (!existsSync(dir)) {
        ...
        const instanceGwToken = (config.gatewayToken as string) ?? generateToken();
        writeInstanceEnv(dir, envConfig);
        writeInstanceCompose(dir, instance.name, port, mcpServers, instanceGwToken);
        request.log.info({ instanceId: id, name: instance.name }, 'Re-provisioned instance dir (was missing after migration)');
      } else {
        // Ensure compose has current spec (e.g. agent-state volume for memory persistence)
        ensureAgentStateDir(dir);
        // Extract gateway token from existing .env for proxy auth
        ...
        writeInstanceCompose(dir, instance.name, 0, mcpServers, gwToken);
      }
      ...
      // Grab container ID if already available (best-effort)
      let containerId: string | null = null;
      try {
        containerId = execSync(
      ...
      // Set running immediately — frontend polls liveStatus via GET to track actual container state
      await prisma.instance.update({
        where: { id },
        data: { status: 'running', containerId },
      });
  ...
  app.post('/api/instances/:id/stop', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      // Set stopped immediately so frontend reflects the change
      await prisma.instance.update({
        where: { id },
        data: { status: 'stopped', containerId: null },
      });

      // Graceful stop in background — 25s SIGTERM grace for memory flush, then SIGKILL.
      // Also stop the MCP proxy sidecar if running.
  ...
  app.post('/api/instances/:id/restart', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      // Restart in background — frontend polls liveStatus to track container state
      const containerName = `wooblay-agent-${instance.name}`;
      exec(`cd "${dir}" && docker compose restart`, { timeout: 60_000 }, (err) => {
        if (err) {
          request.log.warn({ containerName, err: err.message }, 'Background restart warning');
          return;
        }
        // Update container ID after restart completes
        try {
          const newId = execSync(
            `docker ps -q --filter "name=${containerName}"`,
            { timeout: 5000, stdio: 'pipe' },
          ).toString().trim() || null;
          void prisma.instance.update({ where: { id }, data: { containerId: newId } });
        } catch { }
      });
