...
// Subdir inside instance dir where agent state (sessions, memory) is persisted so
// stop/start does not lose context. Mounted at /root/.openclaw in the container.
...
export async function instanceRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/instances/:id/start', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (!existsSync(dir)) {
        ...
        ensureAgentStateDir(dir);
        writeInstanceCompose(dir, instance.name, 0);
      }
  ...
  app.post('/api/instances/:id/stop', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const containerName = `wooblay-agent-${instance.name}`;
      try {
        // -t 25: send SIGTERM, wait up to 25s for graceful exit, then SIGKILL
        // Gives OpenClaw time to persist session/memory to /root/.openclaw (mounted volume)
        execSync(`docker stop -t 25 "${containerName}"`, { timeout: 30_000, stdio: 'pipe' });
      } catch { }
